...
const content = fs.readFileSync(path.join(__dirname, 'src/lib/localLanguageData.js'), 'utf-8');
...
const startPos = content.indexOf('const CURRICULUM_TEXT_EN');
const endPos = content.indexOf('};', startPos);

const entries = Object.entries(newTranslations).sort((a,b) => b[0].length - a[0].length);
const newLines = entries.map(([k,v]) => {
...
const newContent = content.slice(0, endPos) + insertion + '\n' + content.slice(endPos);
fs.writeFileSync(path.join(__dirname, 'src/lib/localLanguageData.js'), newContent, 'utf-8');
console.log(`✅ Added ${entries.length} new translations to CURRICULUM_TEXT_EN`);
